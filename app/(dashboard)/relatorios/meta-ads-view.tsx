import { Megaphone, MessageCircle, MessageSquare, ThumbsDown, ThumbsUp, Trophy } from "lucide-react";
 ...
 */
export async function MetaAdsReportView({ organizationId }: { organizationId: string }) {
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-1 max-w-lg text-sm text-neutral-500 dark:text-neutral-400">
              Leads recebidos via formulário nativo do Facebook/Instagram, agrupados por campanha. Acompanhe a
              jornada completa: formulário → WhatsApp → resposta → qualificação → venda.
            </p>
          </div>
        </div>
        ...
        {rows.length === 0 ? (
          ...
          <>
                  ...
                  {rows.map((row) => {
                    ...
                    return (
                      <tr key={row.campaignId} className="border-b border-neutral-50 last:border-0 dark:border-neutral-900 align-top">
                        <td className="px-3 py-2.5 font-medium text-neutral-900 dark:text-neutral-100">{row.campaignName}</td>
            ...
            <div className="card p-4 text-xs text-neutral-500 dark:text-neutral-400 space-y-1">
              ...
              <div>• <strong className="text-neutral-700 dark:text-neutral-300">Qualificado/Desqualificado:</strong> classificação manual aplicada na página do negócio; cada alteração também dispara um evento &quot;Lead&quot; para a Conversions API da Meta, se a conexão Meta Ads estiver ativa.</div>
            </div>
          </>
        )}
      </div>
