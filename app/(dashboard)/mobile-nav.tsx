...
import { Home, Kanban, MessageCircle, CalendarDays, Menu, X, Plus, Users, BarChart3, Zap, Settings, LogOut } from "lucide-react";
    ...
    <>
      ...
      {sheetOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          />
          <div className="absolute inset-x-0 bottom-0 rounded-t-2xl border-t border-neutral-200 bg-white p-4 pb-[calc(env(safe-area-inset-bottom)+1rem)] shadow-xl dark:border-neutral-800 dark:bg-neutral-900">
            ...
            <div className="space-y-1">
              {OVERFLOW_ITEMS.map((item) => (
                <Link
                  ...
                  href={item.href}
                  onClick={() => setSheetOpen(false)}
                  className="flex items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium text-neutral-700 hover:bg-neutral-50 dark:text-neutral-300 dark:hover:bg-neutral-800"
                >
            ...
            <div className="flex items-center justify-between px-3 py-1">
              <span className="text-sm font-medium text-neutral-700 dark:text-neutral-300">Tema</span>
              <ThemeToggle />
            </div>
            <form action={signOutAction}>
              <button
                type="submit"
                className="mt-2 flex w-full items-center gap-3 rounded-md px-3 py-2.5 text-sm font-medium text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-500/10"
              >
