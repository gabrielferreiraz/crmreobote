...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return runWithTenant(access.organizationId, async () => {
    ...
    if (pipeline._count.processes > 0) {
      return NextResponse.json({ error: "Mova os processos dessa pipeline antes de excluí-la" }, { status: 409 });
    }

    const totalPipelines = await prisma.processPipeline.count({ where: { organizationId: access.organizationId } });
    if (totalPipelines <= 1) {
      return NextResponse.json({ error: "A organização precisa de ao menos uma pipeline de processos" }, { status: 409 });
    }
