...
export async function POST(req: Request) {
  ...
  const {
    ...
    instanceId?: string;
    messageTemplates?: { text: string; weight: number }[];
    delayMinSec?: number;
    delayMaxSec?: number;
  ...
  if (!messageTemplates?.length || messageTemplates.some((t) => !t.text?.trim())) {
    return NextResponse.json({ error: "Escreva ao menos uma variante de mensagem" }, { status: 400 });
  }
