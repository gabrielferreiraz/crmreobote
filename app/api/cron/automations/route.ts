...
import { sendDueMeetingReminders } from "@/lib/tasks/meeting-reminder";
...
// e avisos automáticos antes de Reunião (ver lib/tasks/meeting-reminder.ts)
...
// manual, fora do código, que ninguém lembraria de criar). Os três jobs são
...
async function runCronTick() {
  const [automations, scheduledMessages, meetingReminders] = await Promise.all([
    runAutomations(),
    sendDueScheduledTaskMessages(),
    sendDueMeetingReminders(),
  ]);
  return { automations, scheduledMessages, meetingReminders };
}
